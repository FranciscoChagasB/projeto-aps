...
class RegistroService {
  ...
  Future<Registro> createRegistro(Map<String, dynamic> registroData) async {
    try {
      ...
      throw Exception(
          'Falha ao salvar registro: ${e.response?.data['message']}');
    }
  }
  ...
  Future<Registro> addFeedback(int registroId, String feedback) async {
    try {
      ...
      final response =
          await _dio.patch('/registros/$registroId/feedback', data: feedback);
      return Registro.fromJson(response.data);
    } on DioException {
  ...
  Future<Registro> updateObservacoes(int registroId, String observacoes) async {
    try {
      final response = await _dio.patch('/registros/$registroId/observacoes',
          data: observacoes);
      return Registro.fromJson(response.data);
    } on DioException {
